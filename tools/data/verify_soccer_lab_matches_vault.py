#!/usr/bin/env python3
"""Build and verify the Soccer Lab 2026 matches ex-ante Calyx vault."""
...
SOURCE_SPECS = {
    "swaptr_fifa_wc_2026_matches": {
        "path": Path("swaptr/fifa-wc-2026-matches.zip"),
        "members": {"matches.csv": 85},
    },
    "harrachimustapha_fifa_world_cup_team_dataset": {
        "path": Path("harrachimustapha/fifa-world-cup-team-dataset.zip"),
        "members": {"test.csv": 48, "train.csv": 192},
    },
}
...
class MatchesVaultError(RuntimeError):
...
def source_config(source_id: str) -> dict[str, Any]:
    ...
    for item in data.get("http_files", []):
        if item.get("id") == source_id:
            return item
    raise MatchesVaultError("missing_source_config", {"source_id": source_id})


def ensure_source_zip(raw_root: Path, source_id: str) -> dict[str, object]:
    item = source_config(source_id)
    ...
    if not path.exists():
        ...
        try:
            ...
            raise MatchesVaultError("source_download_failed", {"source_id": source_id, "url": item["url"], "error": str(exc)}) from exc
        if status < 200 or status > 299 or not body:
            raise MatchesVaultError("source_download_bad_response", {"source_id": source_id, "url": item["url"], "status": status, "bytes": len(body)})
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_bytes(body)
        if path.read_bytes() != body:
            raise MatchesVaultError("source_write_readback_mismatch", {"path": str(path.relative_to(ROOT))})
        downloaded = True
    else:
        downloaded = False
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        expected = sorted(spec["members"])
        if members != expected:
            raise MatchesVaultError("unexpected_zip_members", {"source_id": source_id, "expected": expected, "observed": members})
        counts = {}
        for member, expected_rows in spec["members"].items():
            rows = list(csv.DictReader(io.StringIO(archive.read(member).decode("utf-8-sig"))))
            counts[member] = len(rows)
            if len(rows) != expected_rows:
                raise MatchesVaultError("unexpected_source_row_count", {"source_id": source_id, "member": member, "expected": expected_rows, "observed": len(rows)})
...
def generate_rows(raw_root: Path, rows_root: Path) -> dict[str, object]:
    ...
    if proc.returncode != 0:
        raise MatchesVaultError("row_generation_failed", {"stderr": proc.stderr.decode("utf-8"), "stdout": proc.stdout.decode("utf-8")})
    path = rows_root / "matches-2026.jsonl"
    rows = []
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        row = json.loads(line)
        rows.append(row)
        if row["metadata"].get("entity") != "match_2026":
            raise MatchesVaultError("wrong_row_entity", {"metadata": row["metadata"]})
        keys = {token.split("=", 1)[0] for token in row["text"].split()}
        leaked = sorted(forbidden & keys)
        if leaked:
            raise MatchesVaultError("post_match_key_leaked_to_text", {"leaked": leaked, "text": row["text"]})
        anchors = row.get("anchors") or []
        if len(anchors) != 1 or anchors[0].get("kind") != "label:match_result":
            raise MatchesVaultError("missing_match_result_anchor", {"row": row})
        if not anchors[0].get("source") or float(anchors[0].get("confidence", 0)) <= 0:
            raise MatchesVaultError("ungrounded_match_result_anchor", {"anchor": anchors[0]})
        anchor_counts[anchors[0]["value"]] += 1
    if len(rows) != 85:
        raise MatchesVaultError("generated_row_count_mismatch", {"rows": len(rows)})
    return {"row_file": file_stat(path), "rows": len(rows), "anchor_counts": dict(sorted(anchor_counts.items()))}
...
def build_vault(work_dir: Path, rows_path: Path) -> dict[str, object]:
    ...
    vault_name = "soccer-matches-2026"
    create = run_calyx(["create-vault", vault_name, "--panel-template", "text-default"], env)
    if create.returncode != 0:
        raise MatchesVaultError("create_vault_failed", {"stderr": create.stderr.decode("utf-8")})
    ...
    for name, (path, dim) in FACETS.items():
        ...
        if add.returncode != 0:
            raise MatchesVaultError("add_lens_failed", {"facet": name, "stderr": add.stderr.decode("utf-8")})
    ...
    if panel.returncode != 0:
        raise MatchesVaultError("list_panel_failed", {"stderr": panel.stderr.decode("utf-8")})
    ingest = run_calyx(["ingest", vault_name, "--batch", str(rows_path), "--output", "rows"], env, timeout=180)
    if ingest.returncode != 0:
        raise MatchesVaultError("ingest_failed", {"stderr": ingest.stderr.decode("utf-8")[-8000:]})
    ingest_rows = [json.loads(line) for line in ingest.stdout.decode("utf-8").splitlines() if line.strip()]
    if len(ingest_rows) != 85:
        raise MatchesVaultError("ingest_row_count_mismatch", {"rows": len(ingest_rows)})
    readback = run_calyx(
        [
            "readback",
            "cx-list",
    ...
    if readback.returncode != 0:
        raise MatchesVaultError("cx_list_failed", {"stderr": readback.stderr.decode("utf-8")})
    cx_rows = json.loads(readback.stdout)
    if len(cx_rows) != 85:
        raise MatchesVaultError("cx_list_row_count_mismatch", {"rows": len(cx_rows)})
    ...
    for idx, row in enumerate(rows):
        ...
        for facet, slot_id in slot_map.items():
            if slot_id not in by_slot:
                raise MatchesVaultError("missing_expected_slot", {"row": idx, "facet": facet, "slot": slot_id, "available": sorted(by_slot)})
            ...
            if dim != expected_dims[facet]:
                raise MatchesVaultError("slot_dim_mismatch", {"row": idx, "facet": facet, "slot": slot_id, "expected": expected_dims[facet], "observed": dim, "entry": entry})
            slot_counts[facet] += 1
...
def physical_vault_readback(vault_path: Path, slot_map: dict[str, int]) -> dict[str, object]:
    ...
    for name, path in required.items():
        if not path.exists():
            raise MatchesVaultError("missing_physical_vault_file", {"name": name, "path": str(path.relative_to(ROOT))})
        required_stats[name] = file_stat(path)
    slot_files = {}
    for facet, slot_id in slot_map.items():
        cf_files = sorted((vault_path / "cf" / f"slot_{slot_id:02d}").glob("*.sst"))
        dense_index = sorted((vault_path / "idx" / "search").glob(f"slot_{slot_id:05d}_seq_*_n_0000000085.flatdense.bin"))
        if not cf_files or not dense_index:
            raise MatchesVaultError("missing_slot_physical_files", {"facet": facet, "slot": slot_id, "cf_files": len(cf_files), "dense_index": len(dense_index)})
        slot_files[facet] = {
            "slot": slot_id,
...
def main() -> int:
    ...
    report_path = resolve(args.out) if args.out else ROOT / "scratchpad" / "wc2026" / "fsv" / "matches_vault" / "report.json"
    work_dir = report_path.parent
    rows_root = work_dir / "rows"
    sources = {source_id: ensure_source_zip(raw_root, source_id) for source_id in SOURCE_SPECS}
    generation = generate_rows(raw_root, rows_root)
    vault = build_vault(work_dir, rows_root / "matches-2026.jsonl")
    report = {
        "status": "ok",
        "sources": sources,
    ...
    if report_path.read_text(encoding="utf-8") != encoded + "\n":
        raise MatchesVaultError("report_readback_mismatch", {"path": str(report_path)})
